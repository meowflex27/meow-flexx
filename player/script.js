...
function updateIframeSrc() {
  ...
  const queryParams = [
    ...
    "server=2",
    "subtitles=true",
    "sub.en=1" // attempt to force English subs if supported
  ];

  if (baseUrl.includes("vidsrc.cc/v2")) {
    // Server 1 (vidsrc): supports more detailed config
    finalUrl = `${baseUrl}/${id}?${queryParams.join("&")}&lang=en&subtitle=en`;
  } else if (
    baseUrl.includes("vidify.top") ||
    baseUrl.includes("vidfast.pro") ||
    baseUrl.includes("videasy.net")
  ) {
    // Servers 2–5: assume similar structure
    finalUrl = `${baseUrl}/${id}?${queryParams.join("&")}`;
  } else if (baseUrl.includes("/tv/")) {
    // Handle /tv/{id}/{season}/{episode}
    const urlParts = baseUrl.split("/");
    const showId = urlParts[urlParts.length - 3];
    const season = urlParts[urlParts.length - 2];
    const episode = urlParts[urlParts.length - 1];
    finalUrl = `${baseUrl}?${queryParams.join("&")}&lang=en&subtitle=en`;
  } else {
...
if (id) {
  ...
  updateIframeSrc();
  if (downloadBtn) {
    downloadBtn.href = `https://download-weld-kappa.vercel.app/movie?id=${id}`;
  }
} else {
  document.body.innerHTML = `<div class="error">Missing movie ID</div>`;
