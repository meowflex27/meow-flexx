...
// Load Poster and Logo (Trailer Removed)
...
async function showTrailer(id, lazy = false) {
  ...
  if (!id) {
    container.innerHTML = `<p class="no-trailers">Missing movie ID.</p>`;
    return;
  }
  ...
  try {
    ...
    const trailers = data.results.filter(
      video => video.site.toLowerCase() === 'youtube' && video.type.toLowerCase().includes('trailer')
    );
    ...
    // Create trailer thumbnails
    const trailerThumbnails = trailers.map(trailer => `
      <div class="trailer-thumb" data-key="${trailer.key}" style="cursor:pointer;" title="${trailer.name}">
        <img src="https://img.youtube.com/vi/${trailer.key}/hqdefault.jpg" alt="${trailer.name}" />
      </div>
    `).join('');

    // Inject carousel into your container
    labelContent.innerHTML = `
    ...
    // Add click events to play trailer in modal
    document.querySelectorAll('.trailer-thumb').forEach(el => {
      el.addEventListener('click', () => {
        ...
        // Use youtube-nocookie to avoid Error 153
        frame.src = `https://www.youtube-nocookie.com/embed/${key}?autoplay=1&controls=1&rel=0`;
    ...
    // Initialize carousel scroll (if you have a scroll function)
...
function closeTrailerModal() {
  ...
  labelFrame.src = ""; // Stop trailer playback
}

labelClose.addEventListener("click", closeTrailerModal);
window.addEventListener("click", (e) => {
  if (e.target === labelModal) closeTrailerModal();
});
