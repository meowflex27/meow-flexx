...
if (id) {
  ...
  if (downloadBtn) {
    ...
    downloadBtn.href = `https://download-weld-kappa.vercel.app/tv?id=${id}/${currentSeason}/${currentEpisode}`;
  }
} else {
...
function updateIframeSrc(season = 1, episode = 1) {
  ...
  const subtitleParams = [
    ...
    "server=2",
    "subtitles=true",
    "sub.en=1",     // attempt to force English subtitle
    "lang=en",
    "subtitle=en"
  ];

  if (baseUrl.endsWith("/tv") || baseUrl.endsWith("/tv/")) {
    embedURL = `${baseUrl}/${id}/${season}/${episode}?${subtitleParams.join("&")}`;
  } else {
    embedURL = `${baseUrl}/${id}?s=${season}&e=${episode}&${subtitleParams.join("&")}`;
  }
  ...
  if (downloadBtn) {
    downloadBtn.href = `https://download-weld-kappa.vercel.app/tv?id=${id}/${season}/${episode}`;
  }
...
// Example: extract `id` from current page URL
const urlParams = new URLSearchParams(window.location.search);
const tmdbId = urlParams.get("id");

if (tmdbId && backBtn) {
  backBtn.href = `tv-shows-library.html?id=${tmdbId}`;
} else {
  // Fallback if no ID is found
  backBtn.href = "tv-shows-library.html";
}
